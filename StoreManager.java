...
public class StoreManager {
	...
	public static void insertToInventory(int user_id, int item_id) {
		try (Connection conn =  PowPatrolConn.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT inventory_id FROM user_inventory WHERE item_id = ?")) {
			ps.setInt(1, item_id);
			...
			}else { //if the user inventory doesn't have the item yet, it inserts the purchased items
